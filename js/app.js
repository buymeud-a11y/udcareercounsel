// Sample of the 40 questions based on your provided PDF. 
// You can expand this array with the remaining questions.
const questions = [
...
function startTest() {
    ...
    if (!stream) return alert("Please select a stream to continue.");
...
async function finishTest() {
    ...
    const secondHighestScore = sortedTraits[1][1];
    
    // Conflict detection if top two traits are very close
    const isConflicted = (highestScore - secondHighestScore) <= 1;
    ...
    try {
        ...
        console.error(err);
        document.getElementById('loading').classList.add('hidden');
        document.getElementById('report-content').innerHTML = `<div class="p-4 bg-red-100 text-red-700 rounded-lg">Error generating report. Please check your API keys or try again later.</div>`;
        document.getElementById('reset-btn').classList.remove('hidden');
    }
}
